...
exports.main = async (event, context) => {
  ...
  if(event.CDBtype){
    let alldata = []
    let row = [
      "match_type","match_code","team_number","team_role","auto_if_out_line","auto_shoot_lower","auto_shoot_upper","tele_jikuu_times","tele_shoot_lower","tele_shoot_upper","last_climb_stair","winorloss","othertext","who_record"]
    alldata.push(row)  // 列名导入到数组中
    for(let i in tasks){
      let newitem = []
      for(let j=0;j<row.length;j++){
        newitem = newitem.concat(tasks[i][row[j]])
      }
