...
exports.main = async (event, context) => {
  ...
  if(event.CDBtype==true){  
    ...
    let row = ["match_type","match_code","team_number","team_role","auto_if_out_line","auto_shoot_lower","auto_shoot_upper","tele_fangui_times","tele_shoot_lower","tele_shoot_upper","last_climb_stair","winorloss","othertext","who_record","_id"]
    for(let i in tasks){
      for(let m in tasks[i].data){
        let newitem = []
        for(let j=0;j<row.length;j++){
